...
async fn get_regeneration_job_status(
    ...
    Ok(Json(RegenerationStatusResponse {
        ...
        metadata_jobs: job.metadata_jobs,
        metadata_completed: job.metadata_completed,
        thumbnail_jobs: job.thumbnail_jobs,
        thumbnails_completed: job.thumbnails_completed,
        image_text_jobs: job.image_text_jobs,
        image_text_completed: job.image_text_completed,
        started_at: job.started_at.map(|dt| dt.to_rfc3339()),
        completed_at: job.completed_at.map(|dt| dt.to_rfc3339()),
        errors: job.errors,
    }))
