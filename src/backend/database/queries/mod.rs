...
pub mod metadata_jobs {
    ...
    SELECT 'cleaning'
     ...
     HAVING COUNT(*) > 0
    UNION ALL
    SELECT 'waiting_for_rollback', COUNT(*)
      FROM media_metadata_jobs j
     WHERE j.status = 'queued'
       AND EXISTS (
           SELECT 1 FROM file_operation_groups g
            WHERE g.owner_kind = 'metadata_generation'
              AND g.owner_id = CAST(j.media_id AS TEXT)
