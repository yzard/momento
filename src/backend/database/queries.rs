...
pub mod faces {
    ...
    pub const UPDATE_GROUP_REPRESENTATIVE: &str = "UPDATE face_groups SET representative_face_id = (SELECT mf.id FROM face_group_members AS fgm JOIN media_faces AS mf ON mf.id = fgm.face_id WHERE fgm.face_group_id = face_groups.id ORDER BY ((mf.x + (mf.width / 2.0) - 0.5) * (mf.x + (mf.width / 2.0) - 0.5)) + ((mf.y + (mf.height / 2.0) - 0.5) * (mf.y + (mf.height / 2.0) - 0.5)) ASC, mf.frontality DESC, mf.quality DESC, mf.confidence DESC, mf.id ASC LIMIT 1) WHERE id = ?";
    ...
    pub const SELECT_CROP: &str = "SELECT mf.crop_path FROM face_group_members AS fgm JOIN media_faces AS mf ON mf.id = fgm.face_id JOIN media_access AS ma ON ma.media_id = mf.media_id LEFT JOIN face_groups AS fg ON fg.id = fgm.face_group_id WHERE fgm.face_group_id = ? AND ma.user_id = ? AND ma.deleted_at IS NULL ORDER BY CASE WHEN mf.id = fg.representative_face_id THEN 0 ELSE 1 END, ((mf.x + (mf.width / 2.0) - 0.5) * (mf.x + (mf.width / 2.0) - 0.5)) + ((mf.y + (mf.height / 2.0) - 0.5) * (mf.y + (mf.height / 2.0) - 0.5)) ASC, mf.frontality DESC, mf.quality DESC, mf.confidence DESC, mf.id ASC LIMIT 1";
