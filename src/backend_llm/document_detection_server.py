...
def text_layout_metrics(text_regions):
    if not text_regions:
        return 0.0, 0.0
    occupied_area = sum(region["width"] * region["height"] * region["confidence"] for region in text_regions)
    ...
    gaps = [current - previous for previous, current in zip(averaged_lines, averaged_lines[1:])]
    mean_gap = sum(gaps) / len(gaps)
    gap_variance = sum((gap - mean_gap) ** 2 for gap in gaps) / len(gaps)
    regular_spacing = bounded_score(1.0 - math.sqrt(gap_variance) / max(mean_gap, 0.001), "line spacing score")
    left_positions = [region["x"] for region in text_regions]
    mean_left = sum(left_positions) / len(left_positions)
    left_variance = sum((position - mean_left) ** 2 for position in left_positions) / len(left_positions)
    left_alignment = bounded_score(1.0 - math.sqrt(left_variance) / 0.20, "left alignment score")
    regularity_score = line_count_score * (0.60 * regular_spacing + 0.40 * left_alignment)
...
if __name__ == "__main__":
    serve_detection(classify_document, None)
