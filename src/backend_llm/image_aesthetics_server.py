...
def aesthetic_score(raw_rating):
    ...
    return bounded_score((float(raw_rating) - AESTHETIC_MINIMUM_RATING) / rating_range, "aesthetic score")
...
def image_quality_scores(image):
    from PIL import ImageFilter, ImageStat

    grayscale = resize_for_analysis(image, 512).convert("L")
    ...
    edge_mean = float(edge_statistics.mean[0]) / 255.0
    brightness = float(statistics.mean[0])
    edge_variance = float(edge_statistics.var[0])
    histogram = grayscale.histogram()
    pixel_count = sum(histogram)
    if pixel_count <= 0:
        raise RuntimeError("image quality analysis is empty")
    ...
    metrics = (entropy, edge_mean, brightness, edge_variance, exposure, sharpness, unclipped)
    if not all(math.isfinite(metric) for metric in metrics):
        raise RuntimeError("image quality metrics are not finite")
    return {
        "simplicityScore": bounded_score(1.0 - complexity, "simplicity score"),
        "landscapeScore": landscape_score(image),
        "technicalQualityScore": bounded_score(
            (0.45 * sharpness) + (0.35 * exposure) + (0.20 * unclipped), "technical quality score"
        ),
    }
...
def aspect_preserving_square(image, resolution):
    ...
    canvas.paste(resized, ((resolution - resized_width) // 2, (resolution - resized_height) // 2))
    return canvas
...
class PreparedAestheticsInput:
    model_input: Any
    quality_scores: dict[str, float]


def create_aesthetics_responses(
    prepared_inputs: list[PreparedAestheticsInput], raw_model_scores: list[list[float]]
) -> list[dict[str, float]]:
    if len(raw_model_scores) != len(prepared_inputs):
        raise RuntimeError("CLIP returned a different number of scores than prepared inputs")

    responses = []
    for prepared_input, model_scores in zip(prepared_inputs, raw_model_scores):
        if len(model_scores) != 2:
            raise RuntimeError("CLIP returned an invalid aesthetics score shape")
        raw_aesthetic_rating, raw_scenic_score = model_scores
        response = {
            "aestheticScore": aesthetic_score(raw_aesthetic_rating),
            "scenicScore": bounded_score(raw_scenic_score, "scenic score"),
            **prepared_input.quality_scores,
        }
...
class ImageAestheticsRuntime:
    def __init__(
        ...
        self.torch = torch
        self.device = select_cuda_device(requested_device, torch, "image aesthetics")
        clip_model_path = require_model_file(clip_model_path, "CLIP model")
        aesthetic_head_path = require_model_file(aesthetic_head_path, "LAION aesthetic head")
    ...
    def _create_scenic_text_features(self, clip_module: Any) -> Any:
    ...
    def _prepare_input(self, image_source: Any) -> PreparedAestheticsInput:
        image = decode_image(image_source)
        return PreparedAestheticsInput(
            model_input=prepare_clip_tensor(image, self.input_resolution, self.torch),
            quality_scores=image_quality_scores(image),
        )

    def _infer_model_batch(self, prepared_inputs: list[PreparedAestheticsInput]) -> list[dict[str, float]]:
        model_input = self.torch.cat([prepared_input.model_input for prepared_input in prepared_inputs], dim=0).to(
            self.device
        )
        with self.torch.inference_mode():
            ...
            expected_shape = (len(prepared_inputs), CLIP_EMBEDDING_DIMENSIONS)
            if embedding.shape != expected_shape:
                raise RuntimeError(f"CLIP returned embedding shape {tuple(embedding.shape)}")
            raw_aesthetic_ratings = self.aesthetic_head(embedding).squeeze(1)
            scenic_probabilities = self.torch.softmax(
                SCENIC_LOGIT_SCALE * embedding @ self.scenic_text_features.T, dim=1
            )
            raw_model_scores = (
                self.torch.stack((raw_aesthetic_ratings, scenic_probabilities[:, 0]), dim=1).to("cpu").tolist()
            )
