...
describe("MediaDetails", () => {
  it("shows the media ID separately without exposing the stored filename", () => {
    render(<MediaDetails media={{ ...media, id: 9260 }} />);
    expect(screen.getByText("Media ID").parentElement?.textContent).toBe(
      "Media ID9260",
    );
    expect(screen.queryByText("Stored Filename")).toBeNull();
    expect(screen.queryByText(media.filename)).toBeNull();
    expect(
      screen.getByRole("heading", { name: media.originalFilename }),
    ).toBeTruthy();
  });
  ...
  it("shows reverse-geocoded fields as one location row above GPS", () => {
    ...
    expect(
      locationLabel.compareDocumentPosition(gpsLabel) &
