...
vi.mock('../../../src/frontend/components/timeline/PhotoGrid', () => ({
  default: ({ media, onPhotoClick }: { media: Array<{ id: number }>; onPhotoClick: (media: { id: number }) => void }) => {
    ...
    return lastMedia ? <button type="button" onClick={() => onPhotoClick(lastMedia)}>Open last media</button> : null
  },
}))
...
describe('Places page', () => {
  ...
  it('loads subsequent place pages when the sentinel approaches', async () => {
    ...
    expect(mocks.list).toHaveBeenNthCalledWith(2, { cursor: 'place-100', limit: 100 })
  })
  ...
  it('accumulates detail media in API order and opens the existing lightbox at the selected index', async () => {
    ...
    await waitFor(() => expect(mocks.photoGrid).toHaveBeenLastCalledWith([{ id: 10 }, { id: 11 }, { id: 12 }]))
    await userEvent.click(screen.getByRole('button', { name: 'Open last media' }))

    expect(mocks.get).toHaveBeenNthCalledWith(2, { placeId: 'paris-france', cursor: 'media-11', limit: 100 })
    expect(mocks.lightbox).toHaveBeenLastCalledWith(expect.objectContaining({ mediaIds: [10, 11, 12], currentIndex: 2 }))
  })
})
