...
cat > "$workspace/build_mobile_clients.sh" <<'MOBILE'
#!/usr/bin/env bash
set -euo pipefail
[[ "$#" -eq 1 ]]
[[ -d "$1" ]]
...
[[ "${local_invocations[0]}" == "mobile:$keystore_dir" ]] || fail "mobile build did not run first with the keystore directory"
...
[[ "${publish_invocations[0]}" == "mobile:$keystore_dir" ]] || fail "publish did not run the mobile build first"
