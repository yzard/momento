...
fn set_thumbnail(pool: &momento_api::database::DbPool, media_id: i64, bytes: &[u8]) {
    ...
    std::fs::write(thumbnail_path, bytes).expect("thumbnail");
}
...
async fn place_cover_uses_hybrid_score_and_detail_paginates() {
    ...
    set_thumbnail(&pool, iconic_media, &[1, 2, 3]);
    set_thumbnail(&pool, cluttered_media, &[4, 5, 6]);
    ...
    set_thumbnail(&pool, new_best_media, &[7, 8, 9]);
...
#[tokio::test]
async fn thumbnail_requires_a_persisted_thumbnail_reference() {
    let (app, pool) = create_test_app();
