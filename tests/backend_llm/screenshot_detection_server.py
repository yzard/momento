...
SPECIFICATION = importlib.util.spec_from_file_location(
    "screenshot_detection_server_source", SOURCE_DIRECTORY / "screenshot_detection_server.py"
)
...
class ScreenshotDetectionServerTests(unittest.TestCase):
    ...
    def test_recognition_is_limited_to_the_top_status_region(self):
        top_region = text_region("", 0.05, 0.02, 0.2, 0.04)
        boundary_region = text_region("", 0.05, 0.11, 0.2, 0.04)
        content_region = text_region("", 0.05, 0.3, 0.2, 0.04)

        self.assertTrue(SCREENSHOT_DETECTION_SERVER.should_recognize_screenshot_region(top_region))
        self.assertTrue(SCREENSHOT_DETECTION_SERVER.should_recognize_screenshot_region(boundary_region))
        self.assertFalse(SCREENSHOT_DETECTION_SERVER.should_recognize_screenshot_region(content_region))
    ...
    def test_synthetic_mobile_ui_is_a_screenshot(self):
        ...
        for row in range(5):
            top = 188 + row * 92
            drawing.rounded_rectangle((24, top, 336, top + 66), radius=10, fill=(255, 255, 255))
    ...
    def test_synthetic_photo_is_not_a_screenshot(self):
        ...
        pixels = random_generator.integers(0, 256, size=(780, 360, 3), dtype=self.numpy.uint8)
